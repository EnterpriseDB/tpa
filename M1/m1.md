...
\Revision 2.0; 2019-02-02; GC: Introducing a second Barman node; many
failure scenarios reworked as appropriate.
...
One option would be to change the configuration so that Barman is
always pointed to a node in the same DC.

However, this means that the local Barman node cannot be ahead of Node
3, and in particular cannot provide older WAL in case it falls behind.

A workaround could be to create a replication slot for Node 3's
upstream connection. This is not optimal, because we are essentially
using Node 1 as a cache storage for slow network connections.

(TO BE VERIFIED) Thanks to the Geo-Redundancy feature, introduced in
Barman 2.5, the Barman node local to the primary node can feed data to
the other Barman node, so that each standby has a local Barman node to
be used for providing older WAL and for efficiently provisioning new
standbys.
...
Note that the two Barman nodes are independent from each other, and
can take backups in turns, to avoid excessive I/O pressure.

\AuthorComment[GC]{

Maybe this will change when introducing Barman's Geo-Redundancy?
...
9. Reconfigure the Barman nodes to point to the new Primary node
...
1. Provision a new database cluster with upgraded operating system
   and/or software
2. Configure the new cluster as a separate ("New") server in Barman
3. Configure the new cluster as a separate ("New") database in PgBouncer
4. Set up Logical Replication using pglogical from the Primary node in
   ...
   PostgreSQL cluster
5. Wait until the Logical Replication process has caught up
...
\AuthorComment[GC]{In Step 8, can we use table statistics to check
which tables have been modified outside replication? It probably depends on whether SPI writes  }
...
3. Create a new Standby Node 3' by restoring the latest backup from
   Barman
4. Reconfigure Node 3' so that it follows Node 4
