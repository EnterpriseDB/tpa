...
Change the configuration so that Barman is always pointed to a node in
the same DC
...
Update the figures so that each repmgr reconfigures all pgbouncers. At
present some figures suggest that repmgr reconfigures only the local
pgbouncer.
...
![Option: Two Barman Nodes\label{ha-2b}](cache/2b.png){ width=85% }
...
Configure each Barman so it points to the local node, as specified in
RM6794. Some earlier text and a related diagram can be retrieved from
commit:a67d241 .
