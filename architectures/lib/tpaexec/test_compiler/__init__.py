...
class Test(object):
    ...
    def compile(spec, options):
        ...
        for h in hosts:
            if not (isinstance(h, dict) and len(h) == 1):
                raise Exception('%s: hosts: entry must map {hostlabel: [conditions]}' % t.id)
                ...
                # - must_equal: foo
                    ...
                    for v in vars:
                        if isinstance(v, str):
                            ...
                            conditions.append("hostvars[item]['%s'] == %s" % (name, v[name]))
                        else:
                            raise Exception('has_vars: entry must be str or {var: value}')
    ...
    def find_custom_step(self, step):
        ...
        if 'step' in step and 'args' in step:
            ...
            exclude_keys = ['hosts']
            potential_step_names = [
                s for s in step.keys()
                    if s not in exclude_keys and
                        (isinstance(step[s], dict) or step[s] is None)
            ]

        step_directories = self.options.get('step_directories', [])
        step_directories = [
            s for s in step_directories if os.path.exists(s)
        ]
...
def write_yaml(outdir, filename, data):
    ...
    with open(os.path.join(outdir, filename), 'w') as f:
        yaml.dump(data, f, explicit_start=True, default_flow_style=False, sort_keys=True)
