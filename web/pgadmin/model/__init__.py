...
class Service(db.Model):
    ...
    server_software = db.Column(
        db.Integer(),
        db.ForeignKey('meta_server_software.id'),
        nullable=False
    )
    config_type = db.Column(
        db.Integer(),
        db.ForeignKey('meta_config_type.id'),
        nullable=False
    )
    config = db.Column(db.String(), nullable=True)
    plan = db.Column(
        db.Integer(),
        db.ForeignKey('meta_plan.id'),
        nullable = False
    )
    description = db.Column(db.String(1024), nullable=True)
    
class Cluster(db.Model):
    """Define a registered Postgres Cluster"""
...
class MetaServerSoftware(db.Model):
    """All the supported server softwares or PG flavors are stored here"""
    __tablename__ = 'meta_server_software'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(128), nullable=False)
    description = db.Column(db.String(1024), nullable=True)
    ...
    def list(self):
        res=[]
        for ss in self.query.order_by('id'):
            res.append({
                'id': ss.id,
                'name': ss.name
            })
...
class MetaPlan(db.Model):
    """All the supported cloud instance types are stored here"""
    __tablename__ = 'meta_plan'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(128), nullable=False)
    description = db.Column(db.String(1024), nullable=True)
    
class MetaConfigType(db.Model):
    """All the supported preset configuration types are stored here"""
    __tablename__ = 'meta_config_type'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(128), nullable=False)
    description = db.Column(db.String(1024), nullable=True)
