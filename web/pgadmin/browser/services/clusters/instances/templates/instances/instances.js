...
function($, _, S, pgAdmin, pgBrowser, alertify) {
  ...
  if (!pgBrowser.Nodes['instance']) {
    ...
    pgAdmin.Browser.Nodes['instance'] = pgAdmin.Browser.Node.extend({
      parent_type: 'cluster',
      type: 'instance',
      dialogHelp: '{{ url_for('help.static', filename='instance_dialog.html') }}',
      label: '{{ _('Instance') }}',
      canDrop: true,
      hasStatistics: true,
      ...
      Init: function() {
        ...
        pgBrowser.add_menus([{
          name: 'create_instance_on_sg', node: 'instance-group', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 1, label: '{{ _('Instance...') }}',
          data: {action: 'create'}, icon: 'wcTabIcon icon-server'
        },{
          name: 'create_instance', node: 'instance', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 3, label: '{{ _('Instance...') }}',
          data: {action: 'create'}, icon: 'wcTabIcon icon-server'
        },{
          name: 'connect_instance', node: 'instance', module: this,
          applies: ['object', 'context'], callback: 'connect_instance',
          category: 'connect', priority: 4, label: '{{ _('Connect Instance') }}',
          icon: 'fa fa-link', enable : 'is_not_connected'
        },{
          name: 'disconnect_instance', node: 'instance', module: this,
          applies: ['object', 'context'], callback: 'disconnect_instance',
          category: 'drop', priority: 5, label: '{{ _('Disconnect Instance') }}',
          icon: 'fa fa-chain-broken', enable : 'is_connected'
        },{
          name: 'reload_configuration', node: 'instance', module: this,
          ...
          name: 'restore_point', node: 'instance', module: this,
          ...
          name: 'change_password', node: 'instance', module: this,
          applies: ['file'], callback: 'change_password',
          label: '{{ _('Change Password...') }}',
          icon: 'fa fa-lock', enable : 'is_connected'
        },{
          name: 'wal_replay_pause', node: 'instance', module: this,
          ...
          name: 'wal_replay_resume', node: 'instance', module: this,
      ...
      callbacks: {
        ...
        connect_instance: function(args){
          var input = args || {},
            obj = this,
            t = pgBrowser.tree,
          ...
          connect_to_instance(obj, d, t, i);
          return false;
        },
        /* Disconnect the instance */
        disconnect_instance: function(args) {
          var input = args || {},
            obj = this,
            t = pgBrowser.tree,
          ...
          alertify.confirm(
            '{{ _('Disconnect the instance') }}',
            S('{{ _('Are you sure you want to disconnect the instance - %%s ?') }}').sprintf(d.label).value(),
            function(evt) {
              $.ajax({
                ...
                success: function(res) {
                  if (res.success == 1) {
                    ...
                    t.removeIcon(i);
                    d.connected = false;
                    d.icon = 'icon-instance-not-connected';
                    t.addIcon(i, {icon: d.icon});
                    obj.callbacks.refresh.apply(obj, [null, i]);
                    if (pgBrowser.instanceInfo && d._id in pgBrowser.instanceInfo) {
                      delete pgBrowser.instanceInfo[d._id]
                    }
                    obj.trigger('instance-disconnected', obj, i, d);
                  }
                  else {
                    try {
        ...
        /* Connect the instance (if not connected), before opening this node */
        beforeopen: function(item, data) {

          if(!data || data._type != 'instance') {
            return false;
          }

          pgBrowser.tree.addIcon(item, {icon: data.icon});
          if (!data.connected) {
            connect_to_instance(this, data, pgBrowser.tree, item);
        ...
        added: function(item, data) {
          ...
          pgBrowser.instanceInfo = pgBrowser.instanceInfo || {};
          pgBrowser.instanceInfo[data._id] = _.extend({}, data);
        ...
        reload_configuration: function(args){
          ...
          alertify.confirm(
            '{{ _('Reload instance configuration') }}',
            S('{{ _('Are you sure you want to reload the instance configuration on %%s?') }}').sprintf(d.label).value(),
            function(evt) {
              $.ajax({
        ...
        restore_point: function(args) {
          ...
          alertify.prompt('{{ _('Enter the name of the restore point to add') }}', '',
           ...
           function(evt, value) {
             // If user has provided a value, send it to the instance
        ...
        change_password: function(args){
          ...
          if(!alertify.changeInstancePassword) {
            ...
            alertify.dialog('changeInstancePassword' ,function factory() {
              return {
                ...
                prepare: function() {
                    //console.log(this.get('instance').user.name);
                  var self = this;
          ...
          alertify.changeInstancePassword(d).resizeTo('40%','52%');
          return false;
        },
        /* Pause WAL Replay */
          ...
          $.ajax({
            ...
            success: function(res) {
              if (res.success == 1) {
                ...
                t.setInode(i);
                t.deselect(i);
                // Fetch updated data from instance
                setTimeout(function() {
                  t.select(i);
                }, 10);
              }
      ...
      model: pgAdmin.Browser.Node.Model.extend({
        ...
        initialize: function(attrs, args) {
          ...
          if (isNew) {
            this.set({'gid': args.node_info['instance-group']._id});
          }
        ...
        schema: [{
          ...
          id: 'gid', label: '{{ _('Instance Group') }}', type: 'int',
          control: 'node-list-by-id', node: 'instance-group',
          mode: ['create', 'edit'], select2: {allowClear: false}
        },{
          id: 'name', label:'{{ _('Name') }}', type: 'text',
          mode: ['properties', 'edit', 'create']
        },{
          id: 'tags', label:'{{ _('Tags') }}', type: 'text',
          mode: ['properties', 'edit', 'create']
        },{
          id: 'instance_type', label: '{{ _('Instance Type') }}', type: 'options',
          mode: ['properties'], visible: 'isConnected',
          'options': [{% for st in instance_types %}
            {label: '{{ st.description }}', value: '{{ st.instance_type }}'},{% endfor %}
    ...
    function connect_to_instance(obj, data, tree, item) {
      var onFailure = function(xhr, status, error, _model, _data, _tree, _item) {

        tree.setInode(_item);
        tree.addIcon(_item, {icon: 'icon-instance-not-connected'});

        alertify.pgNotifier('error', xhr, error, function(msg) {
          setTimeout(function() {
            alertify.dlgInstancePass(
              '{{ _('Connect to Instance') }}',
      ...
      onSuccess = function(res, model, data, tree, item) {
        ...
        if (res && res.data) {
          ...
          // Update 'in_recovery' and 'wal_pause' options at instance node
          tree.itemData(item).in_recovery=res.data.in_recovery;
          tree.itemData(item).wal_pause=res.data.wal_pause;

          _.extend(data, res.data);

          var instanceInfo = pgBrowser.instanceInfo = pgBrowser.instanceInfo || {};
          instanceInfo[data._id] = _.extend({}, data);

          alertify.success(res.info);
          obj.trigger('instance-connected', obj, item, data);
      ...
      // Ask Password and send it back to the connect instance
      if (!alertify.dlgInstancePass) {
        alertify.dialog('dlgInstancePass', function factory() {
          return {
            ...
            callback: function(closeEvent) {
              ...
              if (closeEvent.button.text == "{{ _('OK') }}") {
                ...
                _tree.addIcon(_item, {icon: 'icon-instance-connecting'});
                ...
                _tree.removeIcon(_item);
                _tree.addIcon(_item, {icon: 'icon-instance-not-connected'});
              }
            }
          };
        });
    ...
    function instance_status(instance_id)
    {
      url = "/ping";
      $.post(url)
  ...
  return pgBrowser.Nodes['instance'];
});
